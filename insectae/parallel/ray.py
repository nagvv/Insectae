...
class _ExecutorWithContext:
    ...
    def execute_with_context(fn: Callable[..., Any], args: Tuple) -> Any:
        # operators in general can and will mutate objects in args, but
        # Ray makes them immutable, so we have to copy them to with with them
        args = deepcopy(args)
...
class RayExecutor(BaseExecutor):
    ...
    @staticmethod
    def fill_globals(context: Dict[str, Any], queue: Queue) -> None:
        global worker_context
        worker_context = context
        worker_context["rng"] = queue.get()

    def init(self, context: Dict[str, Any], rng: np.random.Generator) -> None:
        queue = Queue()
        for child_rng in rng.spawn(self.processes):
            queue.put(child_rng)
        self.pool = Pool(
            processes=self.processes,
            initializer=self.fill_globals,
            initargs=(context, queue),
            **self.ray_kwargs,
        )
