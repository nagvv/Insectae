...
@timing
def evaluate(population: List[dict], **xt) -> None:
    # this function can be parallelized by MPI for heavy target functions
    target = xt['target']
    keyx = xt['keyx']
    keyf = xt['keyf']
...
@timing
def foreach(population: List[dict], op, **xt):
    for ind in population:  # parallel loop
        op(ind, **xt)


@timing
def neighbors(population: List[dict], op, P, **xt):
...
@timing
def pairs(population1: List[dict], population2: List[dict], op, **xt):
    for ind1, ind2 in zip(population1, population2):  # parallel loop
        op((ind1, ind2), twoway=False, **xt)


@timing
def pop2ind(population1: List[dict], population2: List[dict], op, **xt):
...
@timing
def reducePop(population: List[dict], extract, op, post, **xt):
